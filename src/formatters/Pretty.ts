...
export class PrettyFormatter implements Formatter {
  ...
  public constructor(options: PrettyFormatterOptions = {}) {
    this.colors = Color.createFormatters(
      options.colors ?? new Map(),
      PrettyFormatterColors
    )
  ...
  protected formatTimestamp(timestamp: number | Date | undefined): string {
    const formatted = fecha.format(
      new Date(timestamp ?? Date.now()),
      this.dateFormat
    )
    let str = ''
    str += Color.get(ColorCode.GRAY)('[')
    str += this.colors.get(ColorKey.LOG_TIMESTAMP)?.(formatted) as string
    str += Color.get(ColorCode.GRAY)(']')
    return str
  }
  ...
  protected formatPid(): string {
    let str = ''
    str += Color.get(ColorCode.GRAY)('[')
    str += this.colors.get(ColorKey.LOG_PROCESS_ID)?.(
  ...
  protected formatError(error: Error): string {
    ...
    return stack
      .replace(
        ...
        (_, typeAndFn, location1, location2) => {
          return `    ${this.colors.get(ColorKey.ERROR_AT)?.(
            'at'
          )} ${this.colors.get(ColorKey.ERROR_TYPE_AND_FUNCTION)?.(
            typeAndFn
